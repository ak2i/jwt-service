...
import { Hono, cors, jose, load } from "./deps.ts";
...
try {
  if (PRIVATE_KEY_PEM) {
    ...
    console.log("Generated temporary public key for development:", exportedPublicKey.substring(0, 64) + "...");
  }
...
app.post("/issue", apiKeyAuth, async (c) => {
  try {
    ...
    const expiration = exp ? new Date(exp * 1000) : DEFAULT_EXPIRATION;
